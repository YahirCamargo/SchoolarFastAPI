...
def read_by_id(user_id:int,db:Session = Depends(get_db)):
...
def update(user_id:int, user_data: UsuarioCrear, db: Session = Depends(get_db)):
...
def erase(user_id:int,db:Session = Depends(get_db)):
